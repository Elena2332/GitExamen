...
public class MasterMind {
	...
	private final String quebien = "¡ENHORABUENA! Has acertado la combinación en ";
	private String losentimos = "LO SENTIMOS. No has conseguido acertar la combinación en la partida nº ";
	private static final String INICIACOLOR = "Escribe la inicial del color número ";
...
pedirJugada (i);
		...
		System.out.println ();
		visualizarSalida(acertada, i,"");
	}

	public String visualizarSalida(boolean acertada, int i,String str) {
		if (acertada) {
	...
	private void pedirJugada (int j) throws IOException {
		int i;
		for (i=0; i<=4; i++) {
			System.out.println (INICIACOLOR + (i+1) +": ");
	...
	// que hay en la jugada que se está comprobando
		...
		heridos = modificarHeridos(numRM, numVM, numAM, numBM, numNM, numMM,
				numGM, numSM, numRComb, numVComb, numAComb, numBComb, numNComb,
				numMComb, numGComb, numSComb, numRJugada, numVJugada,
				numAJugada, numBJugada, numNJugada, numMJugada, numGJugada,
				numSJugada, heridos);
	...
	public int modificarHeridos(int numRM, int numVM, int numAM, int numBM,
			int numNM, int numMM, int numGM, int numSM, int numRComb,
			int numVComb, int numAComb, int numBComb, int numNComb,
			int numMComb, int numGComb, int numSComb, int numRJugada,
			int numVJugada, int numAJugada, int numBJugada, int numNJugada,
			int numMJugada, int numGJugada, int numSJugada, int heridos) {
